...
namespace pipewire {
  ...
  public:
    ...
    int ensure_stream(const platf::mem_type_e mem_type, const uint32_t width, const uint32_t height, const uint32_t refresh_rate, const struct dmabuf_format_info_t *dmabuf_infos, const int n_dmabuf_infos, const bool display_is_nvidia) {
      ...
      if (!stream_data.stream) {
        ...
        // If pipewire supports setting a target object via object serial and we have one, use it.
        if (object_serial != SPA_ID_INVALID) {
          ...
          node = PW_ID_ANY;  // pw_connect_stream will refer to object serial in PW_KEY_TARGET_OBJECT with this value.
        }
#endif
  ...
  public:
    ...
    int init(platf::mem_type_e hwdevice_type, const std::string &display_name, const ::video::config_t &config) {
      ...
      uint64_t pipewire_object_serial = SPA_ID_INVALID;  // Default for invalid stream from pipewire docs
      // Fetch stream info
      if (configure_stream(display_name, pipewire_fd, pipewire_node, pipewire_object_serial) < 0 || (pipewire_node == PW_ID_ANY && pipewire_object_serial == SPA_ID_INVALID)) {
